...
int I(uint32_t instruction){
    ...
    char rs1 = (instruction >> (7+5+3)) & 0x0000001f; //shifts down to rs1 in bottom 5 bits, and then zeros all but last 5 bits, 0x1f = 0001 1111
    uint32_t imm = (instruction >> (7+5+3+5)) & 0x00000fff; //shifts down to imm in bottom 12 bits, and then zeros all but last 12 bits
    */
    InstructionUnion  i;
...
int S(uint32_t instruction){
    ...
    char rs1 = (instruction >> (7+5+3)) & 0x0000001f; //shifts down to rs1 in bottom 5 bits, and then zeros all but last 5 bits, 0x1f = 0001 1111
    char rs2 = (instruction >> (7+5+3+5)) & 0x0000001f; //shifts down to rs2 in bottom 5 bits, and then zeros all but last 5 bits, 0x1f = 0001 1111
    uint32_t imm2 = (instruction >> (7+5+3+5+5)) & 0x0000007f; //shifts down to imm[11:5] in bottom 7 bits, and then zeros all but last 7 bits
    */
    InstructionUnion  s;
    s.instruction = instruction;
    uint32_t imm = ((uint32_t)(s.S_s.imm11_5) << 5) | s.S_s.imm4_0; 
    //cout << (int)opcode << '\n' <<  (int)imm1 << '\n' <<  (int)funct3 << '\n' <<  (int)rs1 << '\n' <<  (int)rs2 << '\n' <<  (int)imm2 << '\n'; 
    return 0;
}
...
int B(uint32_t instruction){
    ...
    char rs1 = (instruction >> (7+5+3)) & 0x0000001f; //shifts down to rs1 in bottom 5 bits, and then zeros all but last 5 bits, 0x1f = 0001 1111
    char rs2 = (instruction >> (7+5+3+5)) & 0x0000001f; //shifts down to rs2 in bottom 5 bits, and then zeros all but last 5 bits, 0x1f = 0001 1111
    uint32_t imm10_5 = (instruction >> (7+5+3+5+5)) & 0x0000003f; //shifts down to imm[10:5] in bottom 6 bits, and then zeros all but last 6 bits
    uint32_t imm12 = (instruction >> (7+5+3+5+5+6)) & 0x00000001; //shifts down to imm[12] in bottom bit, and then zeros all but last bit
    */
    InstructionUnion  b;
    b.instruction = instruction;
    uint32_t imm = ((uint32_t)(b.B_s.imm12) << 12)| ((uint32_t)(b.B_s.imm11) << 11) | ((uint32_t)(b.B_s.imm10_5) << 5) | ((uint32_t)(b.B_s.imm4_1) << 1); //the pieced together imm, under the assuption that imm[0] = 0;
    cout << (int)imm << '\n' << (int)(b.B_s.rs2) << '\n' << (int)(b.B_s.rs1)  << '\n' << (int)(b.B_s.funct3)  << '\n' << (int)(b.B_s.opcode ) << '\n';
    return 0;
}
...
int U(uint32_t instruction){
    ...
    */
    ...
    uint32_t imm = (uint32_t)(u.U_s.imm) << 12;
    //cout <<(int)opcode  << '\n' << (int)rd << '\n' << (int)imm  << '\n';
    return 0;
    }
...
int J(uint32_t instruction){
    ...
    */
    ...
    uint32_t imm = ((uint32_t)(j.J_s.imm20) << 20) | ((uint32_t)(j.J_s.imm19_12) << 12) | ((uint32_t)(j.J_s.imm11) << 11) | ((uint32_t)(j.J_s.imm10_1) << 1); //the pieced together imm, under the assuption that imm[0] = 0;
    //cout << (int)opcode << '\n' << (int)rd << '\n' << (int)imm << '\n'; //<< (int)imm10_1 << '\n' << (int)imm20 << '\n';
    return 0;
}
